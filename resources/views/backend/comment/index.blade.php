    ...
    <div class="table-responsive">
      ...
      <table class="table table-bordered table-hover" id="order-dataTable" width="100%" cellspacing="0">
        ...
        <tbody>
          ...
          <tr>
            <td>{{ $comment->id }}</td>
            <td>{{ $comment->user_info['name'] ?? 'Anonymous' }}</td>
            <td>{{ optional($comment->post)->title ?? 'Untitled Post' }}</td>
            <td>{{ $comment->comment }}</td>
            <td>{{ $comment->created_at ? $comment->created_at->format('M d D, Y g: i a') : 'Unknown Date' }}</td>
            <td>
